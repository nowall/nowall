...
var proxy = module.exports = function(options) {
  ...
  ****************/
  ...
  var decodeBody= function(body) {
    return body.replace(/([^\?]*)((\?)([^\?]*))?/, function(full, path, _, sep, query) {
        if(query){
  ...
  ****************/
  ...
  ProxyRequest.prototype = {
    ...
    request: function(callback) {
      ...
      _options.path = decodeBody(_options.path);
    ...
    decodeReferer: function(referer) {
      return referer.replace(HOST_PATTERN, function(full, scheme, domain, _, suffix){
          ...
          return scheme + decodeHost(domain) + (suffix && decodeBody(suffix) || '');
      });
    },
    ...
    write: function(serverResponse) {
      ...
      if (zipStream) {
        ...
        .addListener('end', function() {
            ...
            body = encodeBody(body, options, isScript);
            that.headers['content-length'] = Buffer.byteLength(body);
            serverResponse.writeHead(
