...
module.exports = function(options) {
  ...
  var SCRIPT_BACKSLASH_URL_PATTERN = /(http|https|):?\\?\/\\?\/([\w\d\.\-\\]+\.\w{2,4})(\\?\/[^'\";,]*?)(\\?['\"$;,])/ig;
  // to match          var a = 'http:\/\/test.com\/foo\/bar\/'
  // most for youtube
  ...
  function parseUrl (url) {
    ...
    ret.pathquery = ret.path + ret.querystring;
    return ret;
  ...
  function encodeFullUrl(url) {
    ...
    for(var i in whiteList) {
      ...
      if(domain.indexOf('nowall.be') >= 0 ||
        domain.lastIndexOf(w) == domain.length - w.length)
        return url;
    }
  ...
  function encodeUrlPath(url, reqOptions) {
    if(!reqOptions) return encodeFullUrl(url);
