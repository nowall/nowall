...
exports.isText = function (contentType, path) {
  return /(text|javascript|json|xml)/i.test(contentType) || /\.(html|xml|js|json|css|txt)(\?|#|$)/i.test(path);
}

exports.isImage = function (contentType, path) {
  return /(image)/i.test(contentType) || /\.(jpg|png|gif)(\?|#|$)/i.test(path);
}

exports.isVIPResource = function(creq, cres) {
  // download request
  if(creq.headers['Range'] || creq.headers['range']) return true;
  var contentType = cres.headers['content-type'];
  var contentLength = cres.headers['content-length'];
  var path = creq.path;
  return !(/(text|javascript|json|xml|image|font)/i || /\.(html|xml|js|json|css|txt|jpg|png|gif)(\?|#|$)/i.test(path)) || contentLength > 1000000;
}
